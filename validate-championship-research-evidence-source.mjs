...
for(const file of [QUEUE,path.join(DIR,'schema-v1.json')])if(!fs.existsSync(file))throw new Error(`Championship research evidence validation: missing ${file}.`);
const queue=JSON.parse(fs.readFileSync(QUEUE,'utf8'));
const queuedById=new Map((queue.queue||[]).map(c=>[c.clubId,c]));
...
for(const name of files){
  ...
  if(!queued){errors.push(`${name}: club is not in the Championship research queue`);continue;}
  ...
  const expected=new Set((queued.players||[]).map(p=>p.footballName));
  for(const n of names)if(!expected.has(n))errors.push(`${name}: ${n} is not in the staged identity 16`);
  for(const n of expected)if(!Object.prototype.hasOwnProperty.call(rows,n))errors.push(`${name}: staged player ${n} is missing research evidence`);
  for(const [footballName,p] of Object.entries(rows)){
    players++;
    const at=`${name}/${footballName}`;
...
console.log(`Championship research evidence validation PASS · ${files.length} complete club evidence file(s) · ${players} researched players · no fabricated/partial packs admitted.`);
