...
const LEAGUE_ONE_INTAKE='football-db/league-one-research-intake.json';
...
const leagueOneIntake=fs.existsSync(LEAGUE_ONE_INTAKE)?JSON.parse(fs.readFileSync(LEAGUE_ONE_INTAKE,'utf8')):null;
...
let plClubs=0,champClubs=0,leagueOneClubs=0,leagueTwoClubs=0,totalPlayers=0;
for(const [clubId,club] of Object.entries(packs.clubs||{})){
  ...
  if(d===0)plClubs++;else if(d===1)champClubs++;else if(d===2)leagueOneClubs++;else if(d===3)leagueTwoClubs++;
  const identityNames=d===1||d===2?new Set((identities.clubs?.[clubId]||[]).map(p=>p.footballName)):null;
  if(d===1||d===2)check(`${clubId} identity 16`,identityNames.size===16,`${identityNames.size} published identity names`);
  for(const [name,p] of entries){
    ...
    if(d===1||d===2)check(`${clubId}/${name} identity match`,identityNames.has(name),`player must match the published division identity 16 exactly`);
...
check('No League Two research yet',leagueTwoClubs===0,`${leagueTwoClubs} League Two packs`);
check('Pack count',packs.packCount===plClubs+champClubs+leagueOneClubs,`declared ${packs.packCount}, actual ${plClubs+champClubs+leagueOneClubs}`);
...
const pl=coverage.divisions?.find(d=>d.code===0),champ=coverage.divisions?.find(d=>d.code===1),leagueOne=coverage.divisions?.find(d=>d.code===2);
...
check('League One coverage',Number.isInteger(leagueOne?.identityReady)&&leagueOne.identityReady>=0&&leagueOne.identityReady<=24&&leagueOne?.researchReady===leagueOneClubs&&(leagueOneClubs===0||leagueOne.identityReady===24),`identity ${leagueOne?.identityReady}/24, research ${leagueOne?.researchReady}/${leagueOneClubs}`);
...
if(leagueOneIntake)check('League One intake progress',leagueOneIntake.progress?.researchReadyClubs===leagueOneClubs&&leagueOneIntake.progress?.researchPendingClubs===24-leagueOneClubs,`intake ready ${leagueOneIntake.progress?.researchReadyClubs}, pending ${leagueOneIntake.progress?.researchPendingClubs}`);
else check('League One intake absent before research stage',leagueOneClubs===0,`${leagueOneClubs} League One packs exist without an intake`);
...
const result={
  ...
  divisions:{premierLeague:{researchReady:plClubs,players:plClubs*16},championship:{researchReady:champClubs,players:champClubs*16,pending:24-champClubs},leagueOne:{researchReady:leagueOneClubs,players:leagueOneClubs*16,pending:24-leagueOneClubs},leagueTwo:{researchReady:0}},
  totals:{researchClubs:packs.packCount,researchPlayers:totalPlayers,checks:checks.length},
  guarantees:{championshipIdentityReady:24,leagueOneIdentityReady:leagueOne?.identityReady||0,championshipResearchIdentityMatched:true,leagueOneResearchIdentityMatched:true,exactPackSize:16,noFabricatedPromotion:true,teamWriteEnabled:false,careerWriteEnabled:false},
  checks
};
fs.writeFileSync(OUT,JSON.stringify(result,null,2)+'\n','utf8');
manifest.resources=manifest.resources||{};manifest.resources.researchExpansionValidation=OUT;
manifest.coverage=manifest.coverage||{};manifest.coverage.premierLeague=manifest.coverage.premierLeague||{};manifest.coverage.championship=manifest.coverage.championship||{};manifest.coverage.leagueOne=manifest.coverage.leagueOne||{};manifest.coverage.englandClubs=manifest.coverage.englandClubs||{};
...
manifest.coverage.leagueOne.researchPackClubs=leagueOneClubs;manifest.coverage.leagueOne.researchPackPlayers=leagueOneClubs*16;
...
console.log(`England research expansion validation PASS · PL 20/20 · Championship ${champClubs}/24 · League One ${leagueOneClubs}/24 · ${packs.packCount} total packs / ${totalPlayers} players · binary writes locked.`);
