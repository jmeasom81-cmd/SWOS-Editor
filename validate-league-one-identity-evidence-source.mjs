...
const DIR='football-db/league-one-identity-evidence';
...
for(const file of [COVERAGE,path.join(DIR,'schema-v1.json')])if(!fs.existsSync(file))throw new Error(`League One identity evidence validation: missing ${file}.`);
const coverage=JSON.parse(fs.readFileSync(COVERAGE,'utf8'));
const expected=new Set((coverage.clubs||[]).filter(c=>c.division===2).map(c=>c.id));
if(expected.size!==24)throw new Error(`League One identity evidence validation: mapped structure has ${expected.size}/24 clubs.`);
...
for(const file of files){
  ...
  if(!expected.has(clubId))errors.push(`${file}: ${clubId} is not a mapped League One club`);
  ...
  for(const [index,p] of doc.players.entries()){
    ...
    if(!String(p?.source||'').includes('footballsquads.co.uk/eng/2026-2027/flone/'))errors.push(`${at}: current League One source URL is required`);
...
if(errors.length)throw new Error(`League One identity evidence validation failed with ${errors.length} issue(s):\n- ${errors.join('\n- ')}`);
console.log(`League One identity evidence validation PASS · 24 club files · ${players} selected current players · exact-16 and role gates clean.`);
