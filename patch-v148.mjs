...
if(packs.packCount<8||packs.playerCount<128)throw new Error(`SWOS Studio ${BUILD} build failed: expected at least 8 packs / 128 players; found ${packs.packCount} / ${packs.playerCount}.`);
...
if((coverage?.summary?.researchReady||0)<8||(coverage?.summary?.swos16BuilderReady||0)<8)throw new Error(`SWOS Studio ${BUILD} build failed: coverage has not reached the Newcastle eight-pack foundation.`);
...
if(foundationNumber<8)throw new Error(`SWOS Studio ${BUILD} build failed: expected database foundation.8 or later; found ${manifest.version}.`);
...
(function(){
  ...
  function render(){
    ...
    box.innerHTML='<div class="v146-data-head"><div><strong>⚽ Football Data Expansion</strong><p>The production queue is expanding the actual 2026/27 database club by club. Every ready status is generated from validated research files.</p></div><span class="feature-status available">Newcastle United pack published</span></div>'+
      ...
      '<div class="v146-data-foot"><b>Database '+esc(META.databaseVersion)+':</b> Newcastle United is Research Ready and SWOS 16 Builder Ready. The post-window first-team squad supersedes earlier summer snapshots. Summer arrivals keep current value/role evidence without inheriting another club’s 2025/26 output. TEAM.* and .CAR writes remain locked.</div>';
    if(!document.getElementById('v148-release-card')){
      ...
      if(relAnchor){
        ...
        c.innerHTML='<strong>New in v1.48.0 — Newcastle United research pack</strong><span class="about">• Newcastle United becomes the eighth evidence-backed 16-player Premier League research pack, taking the published database foundation to 128 researched players.</span><span class="about">• The pack uses the club’s post-window first-team group after a heavily refreshed summer 2026 squad, avoiding departed-player contamination from earlier snapshots.</span><span class="about">• Newcastle United is Research Ready and SWOS 16 Builder Ready in Club Coverage Centre.</span><span class="about">• TEAM.* and established .CAR writes remain locked.</span>';
...
console.log(`SWOS Studio ${BUILD} Newcastle United research expansion gate complete · ${packs.packCount} packs / ${packs.playerCount} players.`);
