...
if(packs.packCount<7||packs.playerCount<112)throw new Error(`SWOS Studio ${BUILD} build failed: expected at least 7 packs / 112 players; found ${packs.packCount} / ${packs.playerCount}.`);
...
if((coverage?.summary?.researchReady||0)<7||(coverage?.summary?.swos16BuilderReady||0)<7)throw new Error(`SWOS Studio ${BUILD} build failed: coverage has not reached the Everton seven-pack foundation.`);
...
if(foundationNumber<7)throw new Error(`SWOS Studio ${BUILD} build failed: expected database foundation.7 or later; found ${manifest.version}.`);
...
(function(){
  ...
  function render(){
    ...
    box.innerHTML='<div class="v146-data-head"><div><strong>⚽ Football Data Expansion</strong><p>The production queue is expanding the actual 2026/27 database club by club. Every ready status is generated from validated research files.</p></div><span class="feature-status available">Everton pack published</span></div>'+
      ...
      '<div class="v146-data-foot"><b>Database '+esc(META.databaseVersion)+':</b> Everton is Research Ready and SWOS 16 Builder Ready. The post-window squad remains authoritative. TEAM.* and .CAR writes remain locked.</div>';
    if(!document.getElementById('v147-release-card')){
      ...
      if(relAnchor){
        ...
        c.innerHTML='<strong>New in v1.47.0 — Everton research pack</strong><span class="about">• Everton became the seventh evidence-backed 16-player Premier League research pack, establishing the 112-player database foundation.</span><span class="about">• The pack follows the Premier League squad submitted after the summer transfer window, rather than an earlier summer snapshot.</span><span class="about">• Everton is Research Ready and SWOS 16 Builder Ready in Club Coverage Centre.</span><span class="about">• The Research Workbench continues automatically to the next identity-ready club.</span><span class="about">• TEAM.* and established .CAR writes remain locked.</span>';
...
console.log(`SWOS Studio ${BUILD} Everton research expansion gate complete · ${packs.packCount} packs / ${packs.playerCount} players.`);
