...
const DIR='football-db/league-one-research-evidence';
for(const file of [IDENTITIES,COVERAGE,path.join(DIR,'schema-v1.json')])if(!fs.existsSync(file))throw new Error(`League One research evidence validation: missing ${file}.`);
const identities=JSON.parse(fs.readFileSync(IDENTITIES,'utf8'));
const coverage=JSON.parse(fs.readFileSync(COVERAGE,'utf8'));
const leagueOneIds=new Set((coverage.clubs||[]).filter(c=>c.division===2).map(c=>c.id));
if(leagueOneIds.size!==24)throw new Error(`League One research evidence validation: expected 24 League One club IDs, found ${leagueOneIds.size}.`);
...
for(const name of files){
  ...
  if(!leagueOneIds.has(id)){errors.push(`${name}: ${id} is not a League One club`);continue;}
...
if(errors.length)throw new Error(`League One research evidence validation failed with ${errors.length} issue(s):\n- ${errors.join('\n- ')}`);
console.log(`League One research evidence validation PASS · ${files.length} complete club evidence file(s) · ${players} researched players · exact identity matching enforced.`);
