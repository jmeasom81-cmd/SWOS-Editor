...
const QUEUE='football-db/league-one-identity-expansion-queue.json',DIR='football-db/league-one-identity-evidence',OUT='football-db/league-one-identity-intake.json';
if(!fs.existsSync(QUEUE))throw new Error('League One identity intake: queue is missing.');
...
console.log(`League One identity intake · ${out.totals.identityReady}/24 published · ${out.totals.promotionReadyClubs} promotion-ready · next ${out.next?.clubName||'complete'}.`);
