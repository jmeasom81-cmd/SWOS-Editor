...
(function(){
  ...
  function draw(detail){
    ...
    card.innerHTML='<div class="identity-feed-head"><div><strong>🪪 Verified Identity Feed</strong><p>The independent football-data channel carries the verified Premier League identity layer. Studio uses the published feed first and keeps its embedded original foundation only as a fallback.</p></div><span class="feature-status '+(published?'available':'beta')+'">'+(published?'LIVE':'FALLBACK')+'</span></div>'+
  ...
  function releaseNotes(){
    ...
    card.innerHTML='<strong>New in v1.39.0 — Published verified identities</strong><span class="about">• The verified Premier League identity layer is exposed through the independent Football Database.</span><span class="about">• Identity rows are validated for duplicate names, role groups, shirt-number ranges, nationality/source presence and manifest totals.</span><span class="about">• The Premier League squad screen checks the published identity feed before matching names, shirt numbers, broad roles and nationalities.</span><span class="about">• If the feed is unreachable or invalid, Studio safely falls back to the embedded original verified copy.</span><span class="about">• No TEAM.* or .CAR write routines are enabled.</span>';
...
if(feed.clubCount<10||feed.playerCount<215||feed.u21PlayerCount!==19)throw new Error(`SWOS Studio v1.39.0 build failed: identity feed has fallen below the published foundation ${feed.clubCount}/${feed.playerCount}/${feed.u21PlayerCount}.`);
